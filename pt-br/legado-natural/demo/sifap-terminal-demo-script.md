...
| Mensagens canônicas | Não tinha                                  | Adicionadas: "REGISTRO LOCALIZADO COM SUCESSO", "**_ FIM DE LISTAGEM _**", "INFORME O NIS E TECLE ENTER" |
...
- _(Apontar pro `[EXECUTADO]`)_ "Reparem: não é 'OK'. É EXECUTADO. Convenção dos sistemas brasileiros — SIAFI, SIAPE, todos usam essa palavra."
...
- _(Apontar pro prompt)_ "Reparem o prompt: `===>` com **três iguais**. Não é dois, não é um. É padrão Natural — quem viu sistema da década de 90 reconhece na hora."
...
- _(Apontar pro rodapé)_ "Linha de status no final: 'SELECIONE UMA OPCAO E TECLE ENTER'. Era convenção — Natural sempre tinha uma linha de feedback no final da tela. Operador sabia o que fazer só olhando aquela linha."

**[PERGUNTA]** _(opcional)_ "Quem aqui já trabalhou com tela assim de verdade? Levantem a mão."
...
**[TEMPO]** 1 minuto
**[CMD]** `PF5` (ou `F5`)
**[TELA]** Tabela de 3 pagamentos PAGO, todos mesmo banco/agência/conta. Rodapé: "**_ FIM DE LISTAGEM _** 3 REGISTRO(S) LOCALIZADO(S)"
...
**[PERGUNTA]** _(opcional)_ "Onde vocês procurariam essa regra primeiro? No código? Numa documentação que ninguém leu? Falando com um operador prestes a se aposentar?"
...
**[PERGUNTA]** _(opcional)_ "Quantos casos especiais vocês acham que esse `CALC-VALOR-MENSAL` tem? Chutem alto."
...
- _(Apontar pra linha em vermelho)_ "Reparem como o erro aparece: vermelho, na linha de status, código `NAT0850`. Não é popup, não é alerta. É só uma linha que muda. Quem opera no automático nem percebe."
- "No SIFAP real, essas operações só rodam de madrugada, com perfil OPERACAO, e são auditadas."
- "**Decisão de escopo: NÃO vamos modernizar essas duas funções no workshop.** Está fora."
...
- _(Apontar)_ "Reparem: o formulário não sai, não pisca alerta. Só aparece a mensagem em vermelho na linha de status. **Era exatamente assim no SIAFI, no SIAPE, em todo Natural brasileiro.** Você precisa olhar pra linha 24 pra saber o que aconteceu."
...
_Documento de facilitação ao vivo. Use junto com `sifap-terminal.html` v1.1. Parte do material do Legacy Modernization Workshop, Estágio 1: Arqueologia Digital._

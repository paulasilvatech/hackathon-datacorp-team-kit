...
```mermaid
flowchart TD
    subgraph OPEN["Abertura (09:00–09:30)"]
        direction LR
        O1["5 pessoas · 10 personas<br/>Ler TEAM-FLOW.md<br/>Ler os 2 cartões de persona<br/>Copiar os 2 persona-kits para .github/"]
    end

    subgraph S1["Etapa 1 — Arqueologia (13:00–14:15) · @archaeologist"]
        direction TB
        S1A["🔑 RE lidera a exploração<br/>Tech Writer constrói o glossário<br/>EA mapeia o contexto do sistema<br/>DBA lê os DDMs"]
    ...
    subgraph S2["Etapa 2 — Especificação Moderna (14:45–16:00) · @architect"]
        direction TB
        S2A["🔑 SA lidera o design<br/>RE escreve requisitos EARS<br/>EA valida o contexto<br/>PO prioriza o escopo"]
    ...
    subgraph S3["Etapa 3 — Implementação (16:15–17:30) · @builder"]
        direction TB
        S3A["🔑 Dev lidera a codificação<br/>DBA assume o schema<br/>QA escreve testes<br/>TL revisa PRs"]
        ...
        S3C["Serviços Java · Entidades JPA<br/>API REST · Páginas Next.js<br/>Testes verdes · Build verde"]
        S3A --> S3B --> S3C
    end
    ...
    subgraph S4["Etapa 4 — Evolução (17:45–18:15) · @evolution"]
        direction TB
        S4A["🔑 TL distribui issues<br/>DevOps escreve Terraform<br/>QA valida gates de CI<br/>Dev revisa PRs da IA"]
        S4B["/write-github-issue<br/>/delegate-to-copilot-agent<br/>/review-agent-pr<br/>/final-experience-report"]
        S4C["Pipeline CI/CD<br/>Módulo Terraform<br/>PRs da IA revisados<br/>Notas da retro"]
        S4A --> S4B --> S4C
    end

    subgraph CLOSE["Demo + Encerramento (18:00–20:00)"]
        CL1["TL + PO apresentam<br/>Demo de 3 min<br/>Formulários de retro"]
    end

...
- [`../cheat-sheets/specky-workflow.md`](../cheat-sheets/specky-workflow.md) — o plugin gera documentação em cada fase, e você mantém consistência entre o que ele produz e o que o time escreve à mão.
...
1. **(Chat)** *"Revise este README e identifique: seções com TODO, terminologia inconsistente, informação desatualizada (portas, credenciais, endpoints). Proponha correções."*
2. **(Edits)** *"No arquivo ADR-001.md, complete as seções Contexto, Decisão e Consequências usando o template em 02-spec-moderna/ADR-TEMPLATE.md."*
3. **(Chat)** *"Crie um relatório honesto da experiência com Copilot Agent: o que funcionou, o que surpreendeu, o que falhou. Base no template em 04-evolucao/agent-experience-report.md."*
...
- Glossário travado? Pergunte ao Copilot: *"Liste todas as abreviações encontradas nos arquivos `.NSN` do SIFAP e expanda cada uma."*

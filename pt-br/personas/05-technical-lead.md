...
- **Specky** — suporte nas fases 5 (Implementation Plan) e 6 (Task Breakdown).
...
1. **(Chat)** *"Revise este PR: verifique se segue as 3 camadas (domain/application/infrastructure), se o teste cobre happy path + erro, e se há algum import cruzando bounded context."*
2. **(Chat)** *"Temos 3 devs e 3 horas. Features pendentes: [lista]. Crie um plano distribuindo entre devs considerando dependências e complexidade."*
3. **(Chat)** *"`docker compose up` falha com este erro: [cole]. Diagnostique a causa-raiz e proponha um fix."*
